...
const SOURCES = {
  ...
  indonesiaHalal: {
    name: "Endonezya BPJPH — resmî helal ürün ve sertifika kurumu",
    url: "https://bpjph.halal.go.id/",
    type: "certifier" as const,
  },
  chinaHalal: {
    name: "HalalTrip — Çin Müslüman gezgin yemek rehberi",
    url: "https://www.halaltrip.com/country-guide-details/42/china/",
    type: "specialist-directory" as const,
  },
  netherlandsHalal: {
    name: "HalalTrip — Hollanda Müslüman gezgin rehberi",
    url: "https://www.halaltrip.com/country-guide-details/78/netherlands/",
    type: "specialist-directory" as const,
  },
  fruene: {
...
const makePick = (category: "vegan" | "halal", input: PickInput): DietaryRestaurant => {
  const { status, source, verificationNote, lastVerified, ...restaurant } = input;
  const evidence = SOURCES[source];
  return {
    ...restaurant,
    category,
    lastVerified: lastVerified ?? LAST_VERIFIED,
    verification: {
      status,
...
export const DIETARY_GUIDES: DestinationDietaryGuide[] = [
  ...
  // Endonezya — vegan kapsamı HappyCow; helal statüsü BPJPH kaydı görülmeden kesin kabul edilmez.
  guide("ID", "Jakarta", current(v("Burgreens", "Vegan ve bitki bazlı Endonezya", "Menteng", "Endonezya tatlarını, kaseleri ve burgerleri bitki bazlı yorumlayan yerel restoran zinciri.", "Merkezî konumu ve açık bitki bazlı menüsüyle ilk Jakarta gününde kolay, güvenilir bir seçim.", "vegan-focused")), current(h("Sederhana Masakan Padang", "Padang", "Jakarta merkezi", "Nasi Padang ve Sumatra yemekleri sunan yaygın yerel restoran zinciri.", "Müslüman çoğunluklu yerel mutfağı pratik biçimde denemek için; şube sertifikası yine kontrol edilmeli.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Yogyakarta", current(v("Loving Hut Yogyakarta", "Vegan Endonezya ve Asya", "Mergangsan", "Noodle, nasi campur ve bitki bazlı ev yemekleri sunan vegan restoran.", "Kraton–Prawirotaman hattında yerel tatlara yakın geniş bir vegan menü için.")), current(h("Jejamuran", "Mantar odaklı Endonezya", "Sleman", "Mantarla hazırlanan sate, köri ve geleneksel tabakları sunan geniş bahçeli restoran.", "Et ağırlığı olmadan grup yemeği planlamak için; helal sertifika ve sos içerikleri önceden teyit edilmeli.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Ubud", current(v("Zest Ubud", "Vegan Bali ve dünya", "Penestanan", "Yerel ürünlerle renkli, tamamen bitki bazlı tabaklar hazırlayan manzaralı restoran.", "Bali baharatı ile çağdaş vegan mutfağı birleştiren güçlü bir özel akşam seçimi.")), current(h("Warung Makan Bu Rus", "Endonezya ev yemekleri", "Ubud merkezi", "Nasi campur, sebze ve tavuk yemekleri sunan uzun soluklu aile warungu.", "Merkezde alkolsüz ve sade içerik konuşulabilen bir yerel öğün adayı; sertifika varsayılmamalı.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Canggu", current(v("The Shady Shack", "Vejetaryen ve vegan kafe", "Batu Bolong", "Kaseler, burgerler ve tatlılarda geniş vegan seçenekler sunan bahçeli kafe.", "Sörf-kafe gününde birden fazla doyurucu vegan seçeneği aynı menüde bulmak için.", "vegan-focused")), current(h("Warung Dandelion", "Endonezya", "Batu Bolong", "Nasi campur ve yerel ev yemekleri sunan aile işletmesi.", "Canggu'nun uluslararası kafe sahnesi dışında yerel bir öğün için; et tedariki ve alkol yeniden sorulmalı.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Uluwatu", current(v("The Cashew Tree", "Sağlıklı kafe ve vegan seçenekler", "Bingin", "Kaseler, tempeh tabakları ve bitki bazlı ana yemekler sunan açık hava kafesi.", "Bingin plaj günü sonrası besleyici ve içerikleri açık bir akşam için.", "vegan-options")), current(h("Warung Solo Via", "Cava ve Endonezya", "Pecatu", "Nasi campur, çorba ve ızgara sunan gündelik yerel warung.", "Bukit'te uygun bütçeli yerel yemek adayı; BPJPH sertifikası görülmeden kesin helal sayılmamalı.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Nusa Penida", current(v("Vegan Soul Kitchen", "Vegan Endonezya ve dünya", "Ped", "Kaseler, curry, kahvaltı ve tatlıları tamamen bitki bazlı hazırlayan küçük restoran.", "Adadaki sınırlı vegan sahnede açık kapsam ve merkezî kuzey konum sunduğu için.")), current(h("Warung Jukung", "Deniz ürünü ve Endonezya", "Batununggul", "Balık, pirinç ve yerel yemekler sunan kıyı warungu.", "Sade balık ve sebze siparişi konuşulabilen aday; alkol, sos ve ortak mutfak yazılı teyit edilmeli.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Lombok", current(v("Kenza Cafe", "Vegan seçenekli kafe", "Kuta Lombok", "Kahvaltı kaseleri, tempeh ve bitki bazlı ana yemekleri açıkça işaretleyen kafe.", "Güney kıyısında kahvaltıdan akşama birden fazla vegan seçenek için.", "vegan-options")), current(h("El Bazar Cafe & Restaurant", "Akdeniz ve Endonezya", "Kuta Lombok", "Izgara, meze ve yerel yemekler sunan merkezî restoran.", "Müslüman çoğunluklu Lombok'ta et tedariki sorulabilen güçlü aday; alkol servisi nedeniyle teyit şart.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Gili Adaları", current(v("Pachamama Organic Cafe", "Organik ve vegan seçenekler", "Gili Air", "Kaseler, curry ve tatlılarda güçlü vegan seçenekler sunan bahçeli kafe.", "Gili Air'de açık içerikli, hafif ama doyurucu bir öğün için.", "vegan-focused")), current(h("Warung Sunny", "Endonezya ve Sasak", "Gili Air", "Köri, nasi campur ve taze sebze yemekleri sunan aile warungu.", "Yerel Müslüman işletmesi olarak bilinse de güncel BPJPH belgesi ve alkol politikası doğrulanmalı.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Labuan Bajo", current(v("Happy Banana Komodo", "Vegan seçenekli Asya füzyon", "Labuan Bajo merkezi", "Sushi, kaseler ve sebzeli Asya tabaklarında belirgin vegan seçenekler sunan restoran.", "Tekne öncesi veya sonrası açık menüyle bitki bazlı öğün planlamak için.", "vegan-options")), current(h("Taman Laut Handayani", "Endonezya deniz ürünleri", "Labuan Bajo", "Izgara balık ve yerel yemekler sunan geniş restoran.", "Sade deniz ürünü ve alkolsüz sipariş için aday; sertifika, sos ve ortak ızgara teyit edilmeli.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Bromo Dağı", current(v("Café Lava Hostel Restaurant", "Endonezya ve dünya", "Cemoro Lawang", "Menüsünde sebzeli noodle, nasi goreng ve veganlaştırılabilen tabaklar bulunan dağ restoranı.", "Cemoro Lawang'da seçenekler sınırlıyken içerik konuşulabilen pratik bir yemek noktası.", "vegan-options")), current(h("Warung Edi Cemorolawang", "Cava ev yemekleri", "Cemoro Lawang", "Çorba, pirinç ve sıcak Cava tabakları sunan küçük warung.", "Şafak turu çevresinde sade yerel öğün adayı; bağımsız sertifika bulunmadığından açık teyit gerekir.", "verification-recommended", "indonesiaHalal")), "Cemoro Lawang'da hem tam vegan hem bağımsız sertifikalı helal restoran kanıtı sınırlıdır; paketli yedek gıda taşıyın."),
  guide("ID", "Banyuwangi", current(v("Kedai Panorama Banyuwangi", "Endonezya ve vegan seçenekler", "Licin", "Sebze, tofu, tempeh ve pirinç tabakları hazırlanabilen kırsal restoran.", "Ijen rotasında malzemesi konuşulabilen sade bitki bazlı bir öğün adayı.", "vegan-options")), current(h("Rawon Bik Ati", "Doğu Java", "Banyuwangi merkezi", "Rawon, pirinç ve yerel yemekler sunan köklü lokanta.", "Yerel mutfağı denemek için güçlü aday; şube BPJPH kaydı ve et tedariki görülmeden kesin helal sayılmamalı.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Bandung", current(v("Kehidupan Tidak Pernah Berakhir", "Vegan Endonezya", "Bandung merkezi", "Sebze, tofu, tempeh ve bitki bazlı etlerle ekonomik büfe tabakları sunan vegan restoran.", "Sunda kentinde bütçe dostu, kapsamı açık ve yerel damaklı vegan öğün için.")), current(h("Sindang Reret", "Sunda mutfağı", "Bandung", "Nasi timbel, ızgara ve geleneksel Sunda yemekleri sunan köklü restoran.", "Yerel mutfağı aile düzeninde denemek için; güncel BPJPH sertifikası ve şube kapsamı doğrulanmalı.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Surabaya", current(v("Loving Hut Surabaya", "Vegan Endonezya ve Asya", "Surabaya merkezi", "Noodle, pirinç ve bitki bazlı ev yemekleri sunan vegan restoran.", "Et ağırlıklı Doğu Java mutfağı içinde açık kapsamlı bir vegan öğün için.")), current(h("Soto Ayam Ambengan Pak Sadi", "Soto ayam", "Ambengan", "Surabaya usulü tavuk çorbasıyla bilinen köklü lokanta.", "Şehrin imza sıcak yemeğini denemek için; BPJPH belgesi ve tüm sos içerikleri şubeden teyit edilmeli.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Toba Gölü", current(v("Jenny's Restaurant", "Endonezya ve vegan seçenekler", "Tuk Tuk", "Tofu, tempeh, sebze curry ve veganlaştırılabilen yerel tabaklar sunan küçük restoran.", "Samosir'de sınırlı sahnede bitki bazlı içerikleri konuşarak sipariş etmek için.", "vegan-options")), current(h("Rohana's Restaurant", "Endonezya ve göl balığı", "Tuk Tuk", "Balık, pirinç ve sebze yemekleri sunan aile restoranı.", "Sade balık/vegan siparişi için aday; Batak mutfağında domuz ve alkol yaygın olduğundan ayrı hazırlama şart.", "verification-recommended", "indonesiaHalal")), "Samosir'de domuz eti Batak mutfağının önemli parçasıdır; bağımsız helal sertifika kanıtı sınırlı olduğundan ayrılmış ekipman ve içerik yazılı sorulmalıdır."),
  guide("ID", "Bukittinggi", current(v("Turret Cafe", "Endonezya ve vegan seçenekler", "Jam Gadang çevresi", "Sebze, tofu, tempeh ve pirinç tabakları hazırlanabilen merkezî kafe.", "Tam vegan restoranın sınırlı olduğu şehirde malzemeyi konuşarak sipariş verilebildiği için.", "vegan-options")), current(h("Nasi Kapau Uni Lis", "Minangkabau", "Pasar Atas", "Nasi kapau, gulai ve sebze tabakları sunan tanınmış yerel lokanta.", "Müslüman çoğunluklu Minangkabau mutfağını yerinde denemek için; güncel sertifika kaydı yine kontrol edilmeli.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Raja Ampat", current(v("Yenbuba Homestay Mutfağı", "Ev tipi ada yemekleri", "Yenbuba", "Ön bildirimle tofu, tempeh, sebze ve pirinç ağırlıklı öğün hazırlayabilen homestay mutfağı.", "Restoran bulunmayan adalarda vegan ihtiyacını konaklamayla yazılı planlamak için.", "vegan-options")), current(h("Raja Ampat Dive Lodge Restaurant", "Ada ve deniz ürünleri", "Mansuar", "Konaklayan misafirlere balık, pirinç ve önceden konuşulabilen özel öğün sunan tesis restoranı.", "Uzak adada içerikleri rezervasyon öncesi yazılı teyit edebilecek düzenli mutfak alternatifi olduğu için.", "verification-recommended", "indonesiaHalal")), "Raja Ampat'ta halka açık restoran erişimi çok sınırlıdır; vegan ve helal gereksinimini konaklama rezervasyonundan önce yazılı planlayın ve paketli yedek gıda taşıyın."),
  guide("ID", "Makassar", current(v("Eat Vegan", "Vegan Sulawesi ve Endonezya", "Wajo", "Coto Makassar, pallubasa, sate ve nasi goreng'i tamamen bitki bazlı yorumlayan vegan restoran.", "Haziran 2026'da açılan ve yerel Sulawesi tatlarını veganlaştıran güncel, kapsamı açık bir öğün için.")), current(h("Coto Nusantara", "Makassar coto", "Ujung Pandang", "Coto Makassar ve ketupat sunan köklü yerel lokanta.", "Şehrin imza yemeğini denemek için; BPJPH sertifikası, sakatat ve tüm sosların içeriği teyit edilmeli.", "verification-recommended", "indonesiaHalal"))),
  guide("ID", "Tana Toraja", current(v("Kaana Toraya Coffee", "Kafe ve vegan seçenekler", "Rantepao", "Kahve, pirinç, sebze ve veganlaştırılabilen basit tabaklar sunan yerel kafe.", "Rantepao'da sınırlı vegan sahnede açık iletişim kurulabilen bir aday olduğu için.", "vegan-options")), current(h("Rimiko Restaurant", "Endonezya ve Toraja", "Rantepao", "Yerel ve Endonezya yemekleri sunan merkezî restoran.", "Sade balık/sebze siparişi için planlama adayı; domuz, alkol ve ortak mutfak nedeniyle kesin helal değildir.", "verification-recommended", "indonesiaHalal")), "Tana Toraja'da domuz ve törensel et yemekleri yaygındır; doğrulanmış helal restoran sınırlı olduğundan önceden paketli gıda ve yazılı mutfak teyidi önerilir."),

  // Çin — HappyCow ve Müslüman gezgin dizinleri kullanılır; yerel sertifika belgesi görülmeden temkinli statü korunur.
  guide("CN", "Pekin", current(v("King's Joy", "Vegan Çin yüksek mutfağı", "Dongcheng", "Çin sebze geleneğini rafine tadım menülerinde sunan tamamen vejetaryen restoran.", "Pekin'de özel bir akşam için ürün, teknik ve sunum seviyesi en güçlü bitki bazlı adreslerden.", "vegan-focused")), current(h("Ju Bao Yuan", "Pekin Müslüman hotpot", "Niujie", "Kuzu ve dana hotpotuyla bilinen Hui mahallesi restoranı.", "Niujie'de Çin Müslüman mutfağını yerel topluluk içinde deneyimlemek için.", "restaurant-confirmed", "chinaHalal"))),
  guide("CN", "Şanghay", current(v("Duli", "Vegan Asya füzyon", "Jing'an", "Sichuan ve Asya tatlarını tamamen bitki bazlı küçük tabaklarda sunan restoran.", "Şanghay'ın çağdaş yemek sahnesinde paylaşmalı ve karakterli bir vegan akşam için.")), current(h("Hong Chang Xing", "Şanghay Müslüman mutfağı", "People's Square", "Kuzu hotpot ve Hui usulü yemekleriyle bilinen tarihî helal restoran.", "Merkezde uzun geçmişi olan Çin Müslüman mutfağı deneyimi için.", "restaurant-confirmed", "chinaHalal"))),
  guide("CN", "Xi'an", current(v("Tianlong Baoyan Vegetarian Restaurant", "Budist vejetaryen", "Daxingshan Temple çevresi", "Çin sebze, tofu ve mantar yemekleri sunan vejetaryen restoran.", "Et suyu ve hayvansal içerik riskini azaltan geniş yerel bitki bazlı seçenek için.", "vegan-focused")), current(h("Lao Mi Jia Paomo", "Xi'an Hui mutfağı", "Müslüman Mahallesi", "Kuzu paomo ve Hui atıştırmalıkları sunan aile lokantası.", "Şehrin simge yemeğini Müslüman mahalle bağlamında denemek için.", "restaurant-confirmed", "chinaHalal"))),
  guide("CN", "Chengdu", current(v("Mi Xun Teahouse", "Vejetaryen Sichuan", "Taikoo Li", "Sichuan tatlarını zarif vejetaryen setlerde yorumlayan çayevi-restoran.", "Mala aromasını et kullanmadan rafine biçimde deneyimlemek için.", "vegan-focused")), current(h("Chengdu Huangcheng Mosque Restaurant", "Hui ve Sichuan", "Tianfu Square çevresi", "Hui topluluğuna yönelik erişte, kuzu ve yerel yemekler sunan restoran.", "Merkezde domuzsuz Çin yemeği için güçlü aday; güncel sertifika ve ortak mutfak sorulmalı.", "verification-recommended", "chinaHalal"))),
  guide("CN", "Chongqing", current(v("Pure Lotus Vegetarian", "Çin vejetaryen", "Yuzhong", "Tofu, mantar ve sebzelerle Çin usulü tabaklar hazırlayan vejetaryen restoran.", "Hotpot ağırlıklı kentte etten uzak, içerikleri konuşulabilir bir akşam için.", "vegan-focused")), current(h("Yilan Lanzhou Beef Noodles", "Lanzhou Müslüman eriştesi", "Yuzhong", "Elle çekme erişte ve dana çorbası sunan Müslüman lokantası.", "Yüksek yağlı hotpot dışında hızlı ve topluluk temelli helal yemek adayı olduğu için.", "verification-recommended", "chinaHalal"))),
  guide("CN", "Hangzhou", current(v("Su Shan Shu Shi", "Çin vejetaryen", "West Lake çevresi", "Mevsim sebzeleri, tofu ve mantarla rafine Çin tabakları sunan restoran.", "Batı Gölü gününde yerel tekniklerle hazırlanmış kapsamlı bitki bazlı öğün için.", "vegan-focused")), current(h("Phoenix Mosque Muslim Restaurant", "Hui mutfağı", "Shangcheng", "Cami çevresinde erişte, kuzu ve Çin Müslüman yemekleri sunan restoran.", "Tarihî Müslüman topluluğuyla bağlantılı merkezî yemek adayı olduğu için.", "verification-recommended", "chinaHalal"))),
  guide("CN", "Suzhou", current(v("Wu Guan Tang", "Çin vejetaryen", "Gusu", "Sebze, tofu ve mevsimlik Jiangnan yemekleri sunan sakin restoran.", "Bahçe gününü hafif ve yerel tatlı bitki bazlı öğünle tamamlamak için.", "vegan-focused")), current(h("Suzhou Muslim Restaurant", "Hui ve kuzey Çin", "Gusu", "Dana-kuzu, erişte ve hamur işi sunan Müslüman lokantası.", "Domuzsuz bölgesel yemek için aday; yerel sertifika ve mutfak ayrımı yeniden teyit edilmeli.", "verification-recommended", "chinaHalal"))),
  guide("CN", "Nanjing", current(v("Jiming Temple Vegetarian Restaurant", "Budist vejetaryen", "Xuanwu", "Tapınak çevresinde erişte, tofu ve sebze yemekleri sunan vejetaryen restoran.", "Xuanwu–sur rotasında geleneksel, sade bitki bazlı öğün için.", "vegan-focused")), current(h("Ma Xiang Xing", "Nanjing Müslüman mutfağı", "Qinhuai", "Ördek, dana ve geleneksel Hui yemekleriyle bilinen köklü restoran.", "Nanjing'de uzun geçmişi olan Müslüman mutfak kurumunu deneyimlemek için.", "restaurant-confirmed", "chinaHalal"))),
  guide("CN", "Guilin ve Yangshuo", current(v("Nengren Vegetarian Restaurant", "Budist vejetaryen", "Guilin merkezi", "Tofu, sebze ve Çin ev yemekleri sunan sade vejetaryen restoran.", "Guilin'de cruise öncesi açık bitki bazlı öğün bulmak için.", "vegan-focused")), current(h("Guilin Islam Restaurant", "Hui ve kuzey Çin", "Chongshan Road çevresi", "Erişte, kuzu ve dana yemekleri sunan Müslüman restoranı.", "Guilin merkezde cami/topluluk çevresinde helal yemek adayı olduğu için.", "verification-recommended", "chinaHalal"))),
  guide("CN", "Zhangjiajie", current(v("Xiao Xiong Si Fang Cai", "Çin ve vegan seçenekler", "Tianmen teleferiği çevresi", "Ayrı vejetaryen bölümünde patlıcan, salatalık, patates ve tofu gibi çoğu vegan hazırlanabilen yemekler sunan yerel restoran.", "Ağustos 2026'da güncellenen dizin kaydı ve hayvansal içerikleri tek tek konuşmaya izin veren ayrı menüsü için.", "vegan-options")), current(h("Wulingyuan Muslim Restaurant", "Hui ve kuzey Çin", "Wulingyuan", "Erişte, dana ve kuzu yemekleri sunan küçük Müslüman lokantası.", "Park gününde domuzsuz sıcak yemek adayı; güncel işletme ve sertifika önceden doğrulanmalı.", "verification-recommended", "chinaHalal")), "Wulingyuan'da işletme adları ve sezonluk çalışma hızlı değişebilir; paketli yedek gıda taşıyın ve otelden güncel konumu teyit edin."),
  guide("CN", "Guangzhou", current(v("Su He Jing", "Çin vejetaryen", "Yuexiu", "Kanton teknikleriyle sebze, tofu ve mantar tabakları hazırlayan vejetaryen restoran.", "Dim sum kentinde yerel dokuyu koruyan kapsamlı bitki bazlı bir masa için.", "vegan-focused")), current(h("Xinyue Muslim Restaurant", "Uygur ve kuzeybatı Çin", "Yuexiu", "Kuzu şiş, pilav ve erişte sunan Müslüman restoranı.", "Guangzhou merkezinde açık Müslüman mutfak kimliğiyle doyurucu grup yemeği için.", "restaurant-confirmed", "chinaHalal"))),
  guide("CN", "Shenzhen", current(v("Yunlaiju Vegetarian", "Yunnan ve Çin vejetaryen", "Futian", "Mantar, tofu ve sebzeleri Yunnan esintili sunan vejetaryen restoran.", "Teknoloji merkezinde bölgesel Çin tatlarını bitki bazlı denemek için.", "vegan-focused")), current(h("Mevlana Turkish Restaurant", "Türk", "Futian", "Kebap, pide ve mezeler sunan Türk restoranı.", "Uluslararası bölgede helal et beyanı sorulabilen tanıdık mutfak için; sertifika ve alkol yeniden teyit edilmeli.", "verification-recommended", "chinaHalal"))),
  guide("CN", "Kunming", current(v("Qing Xin Vegetarian", "Yunnan vejetaryen", "Wuhua", "Yunnan sebzeleri, tofu ve mantar yemekleri sunan vejetaryen restoran.", "Mantar mutfağını daha kontrollü ve bitki bazlı biçimde denemek için.", "vegan-focused")), current(h("Yilan Yuan", "Yunnan Hui mutfağı", "Kunming merkezi", "Dana, kuzu ve Yunnan-Hui yemekleri sunan bilinen Müslüman restoranı.", "Kunming'in önemli Hui yemek geleneğini merkezî bir adreste denemek için.", "restaurant-confirmed", "chinaHalal"))),
  guide("CN", "Dali", current(v("Yi Ran Tang Vegetarian Restaurant", "Budist vejetaryen", "Dali Ancient Town", "Büfe düzeninde sebze, tofu ve pirinç tabakları sunan vejetaryen restoran.", "Eski şehirde bütçe dostu ve açık bitki bazlı öğün için.", "vegan-focused")), current(h("Dali Muslim Restaurant", "Yunnan Hui", "Dali Ancient Town", "Dana eriştesi, kuzu ve pilav sunan Müslüman lokantası.", "Bai mutfağı yanında domuzsuz sıcak yemek adayı; güncel sertifika ve işletme adı yerinde doğrulanmalı.", "verification-recommended", "chinaHalal"))),
  guide("CN", "Lijiang", current(v("N's Kitchen", "Vegan seçenekli Yunnan ve dünya", "Dayan Old Town", "Tofu, sebze curry ve veganlaştırılabilen ana yemekler sunan kafe.", "Eski şehirde İngilizce içerik iletişimi ve birden fazla bitki bazlı seçenek için.", "vegan-options")), current(h("Lijiang Muslim Restaurant", "Hui ve Yunnan", "Dayan dış kapısı", "Dana eriştesi, kuzu ve pilav sunan Müslüman lokantası.", "Domuzun yaygın olduğu bölgede topluluk odaklı yemek adayı; güncel konum ve sertifika teyit edilmeli.", "verification-recommended", "chinaHalal"))),
  guide("CN", "Harbin", current(v("JingXinLian Vegetarian", "Çin ve Tayvan vejetaryen", "Nangang", "Geniş tofu, sebze ve bitki bazlı et seçkisi sunan; çoğu yemeği vegan hazırlanabilen restoran.", "Aralık 2024'te güncellenen ve 2025–2026 dizinlerinde etkinliği görülen kapsamlı sıcak öğün için.", "vegan-focused")), current(h("Harbin Mosque Muslim Restaurant", "Dongbei Hui", "Daowai", "Dana, kuzu, mantı ve kuzeydoğu Çin yemekleri sunan Müslüman restoranı.", "Harbin'in tarihî Müslüman topluluğu çevresinde sıcak yemek adayı olduğu için.", "verification-recommended", "chinaHalal"))),

  // Hollanda — HappyCow kapsamı ve HalalTrip rehberi; kırsal rotalarda bağımsız sertifika kanıtı sınırlı olabilir.
  guide("NL", "Amsterdam", current(v("Vegan Junk Food Bar", "Vegan comfort food", "De Pijp", "Burger, loaded fries ve renkli fast-food tabaklarını tamamen vegan sunan restoran.", "Amsterdam'ın çağdaş vegan sahnesinde eğlenceli ve kapsamı açık bir akşam için.")), current(h("MOZO", "Fas", "De Pijp", "Tajin, kuskus ve ızgara sunan Fas restoranı.", "Merkezde Müslüman mutfak odağı güçlü bir akşam için; et sertifikası ve alkol politikasını teyit edin.", "verification-recommended", "netherlandsHalal"))),
  guide("NL", "Rotterdam", current(v("Rozey", "Vejetaryen ve vegan paylaşım", "Centrum", "Küçük tabakların çoğunu vegan sunan limitsiz paylaşım restoranı.", "Karışık gruplarda çok sayıda bitki bazlı seçeneği aynı masada denemek için.", "vegan-focused")), current(h("Bazar", "Kuzey Afrika ve Orta Doğu", "Witte de Withstraat", "Meze, tajin ve ızgaralar sunan canlı restoran.", "Rotterdam'ın çok kültürlü merkezinde helal et seçeneği sorulabilen güçlü aday; menünün tamamı varsayılmamalı.", "halal-options", "netherlandsHalal"))),
  guide("NL", "Lahey", current(v("Veggies On Fire", "Vegan dünya mutfağı", "Den Haag merkezi", "Günlük tabaklar ve yaratıcı ana yemekleri tamamen vegan hazırlayan restoran.", "Şehir merkezinde samimi, kapsamı açık bitki bazlı akşam için.")), current(h("Saray", "Türk", "Den Haag merkezi", "Ocakbaşı, pide ve mezeler sunan Türk restoranı.", "Lahey'in güçlü Türk topluluğu içinde helal et beyanı sorulabilen kapsamlı menü için.", "verification-recommended", "netherlandsHalal"))),
  guide("NL", "Utrecht", current(v("Gys", "Organik ve vegan seçenekler", "Voorstraat", "Organik menüsünde açık işaretli çok sayıda vegan ana yemek ve tatlı sunan restoran.", "Kanal gününde yerel ürün ve farklı diyetleri aynı masada buluşturmak için.", "vegan-focused")), current(h("Kasap Utrecht", "Türk ızgara", "Kanaalstraat", "Izgara, pide ve dürüm sunan Türk restoranı.", "Lombok mahallesinde Müslüman topluluk odaklı helal yemek adayı; sertifika güncelliği doğrulanmalı.", "verification-recommended", "netherlandsHalal"))),
  guide("NL", "Haarlem", current(v("Mama Gaia", "Vegan ve sürdürülebilir", "Haarlem merkezi", "Mevsimsel ürünlerle tamamen bitki bazlı paylaşım tabakları sunan restoran.", "Haarlem'de özel ama rahat, yerel ürün odaklı vegan akşam için.")), current(h("Bosphorus Haarlem", "Türk", "Centrum", "Kebap, pide ve mezeler sunan Türk restoranı.", "Merkezde helal et beyanı sorulabilen pratik aile yemeği için; sertifika ve alkol yeniden teyit edilmeli.", "verification-recommended", "netherlandsHalal"))),
  guide("NL", "Leiden", current(v("LAB071", "Sebze odaklı ve vegan seçenekler", "Leiden Bio Science Park", "Yerel, mevsimsel ve fermente ürünlerle geniş, açıkça uyarlanabilen vegan paylaşım tabakları sunan restoran.", "Haziran 2026 yorumlarında geniş vegan kartı doğrulanan, yaratıcı bir akşam için.", "vegan-focused")), current(h("Taratoer", "Lübnan vejetaryen", "Professorenwijk", "Falafel, humus, kebbeh ve çok sayıda vegan Levant tabağı sunan vejetaryen restoran.", "Et sertifikası belirsizliğini ortadan kaldıran, Müslüman işletmeciyle içerik konuşulabilen sebze odaklı alternatif için.", "verification-recommended", "netherlandsHalal"))),
  guide("NL", "Delft", current(v("Kek", "Kafe ve vegan seçenekler", "Delft merkezi", "Vegan pide, tempeh sandviç, fırın ürünü ve bitkisel içecekleri açıkça işaretleyen popüler brunch kafesi.", "Mayıs 2026 yorumlarıyla güncel, merkezî ve birden fazla doyurucu vegan seçenek sunduğu için.", "vegan-options")), current(h("Hanno Groots Café", "Kafe ve dünya", "Delft merkezi", "Vejetaryen, balık ve farklı dünya yemekleri sunan kafe-restoran.", "Sade balık/vegan siparişiyle planlanabilecek aday; bağımsız helal sertifika bulunmadığından kesin kabul edilmemeli.", "verification-recommended", "netherlandsHalal")), "Delft'te merkezî bağımsız helal restoran sertifikası için güçlü kanıt sınırlıdır; Rotterdam veya Lahey'de daha fazla seçenek bulunur."),
  guide("NL", "Maastricht", current(v("Bar Verde", "Vegan comfort food", "Sint Pieter", "Kapsalon, sushi, ravioli, wrap ve tatlılardan oluşan tamamen vegan menü sunan restoran.", "Mayıs 2026 yorumlarıyla etkinliği doğrulanan, akşam yemeği ve tatlı için kapsamlı bir seçenek.")), current(h("Levantini", "Levant", "Maastricht merkezi", "Falafel, humus, meze ve ızgara sunan Levant restoranı.", "Sebze seçenekleri güçlü ve helal et talebi konuşulabilen masa için; sertifika yeniden teyit edilmeli.", "verification-recommended", "netherlandsHalal"))),
  guide("NL", "Groningen", current(v("Achterwerk", "Vegan yaratıcı mutfak", "Groningen merkezi", "Mevsimsel ürünlerle tamamen bitki bazlı yaratıcı menüler hazırlayan restoran.", "Kuzeyin genç yemek sahnesinde özel ve deneysel vegan akşam için.")), current(h("Tekinev", "Türk", "Groningen merkezi", "Türk ev yemekleri, pide ve ızgara sunan restoran.", "Müslüman toplulukla bağlantılı yerel yemek adayı; et sertifikası ve alkol politikası sorulmalı.", "verification-recommended", "netherlandsHalal"))),
  guide("NL", "Eindhoven", current(v("Krishna Vilas", "Güney Hint vejetaryen", "Eindhoven merkezi", "Dosa, idli, curry ve çok sayıda vegan hazırlanabilen Güney Hint yemeği sunan vejetaryen restoran.", "Tam vegan restoran sayısının azaldığı şehirde kapsamı geniş, doğal olarak bitki ağırlıklı bir öğün için.", "vegan-focused")), current(h("Antep Sofrası", "Türk", "Eindhoven", "Kebap, pide ve geleneksel Türk yemekleri sunan restoran.", "Şehirde helal et beyanı sorulabilen geniş aile menüsü için; güncel sertifika teyit edilmeli.", "verification-recommended", "netherlandsHalal"))),
  guide("NL", "Giethoorn", current(v("Smit's Paviljoen", "Kafe ve vegan seçenekler", "Bovenwijde", "Menüsünde bitki bazlı uyarlanabilen salata, ekmek ve sıcak seçenekler bulunan su kenarı restoranı.", "Köyde sınırlı seçenek varken rezervasyonla vegan öğün konuşulabildiği için.", "vegan-options")), current(h("Grand Café Fanfare", "Hollanda ve dünya", "Giethoorn merkezi", "Balık, vejetaryen ve yerel yemekler sunan restoran.", "Sade balık/vegan siparişiyle planlanabilecek aday; helal sertifikası veya ayrı mutfak garantisi yoktur.", "verification-recommended", "netherlandsHalal")), "Giethoorn'da doğrulanmış sertifikalı helal restoran kanıtı sınırlıdır; paketli öğün getirmek veya Zwolle'de yemek daha güvenli olabilir."),
  guide("NL", "Zaanse Schans", current(v("De Kraai", "Pannenkoek ve vegan seçenekler", "Zaanse Schans", "Talep üzerine süt-yumurta içermeyen seçenekleri sorulabilen pancake restoranı.", "Gezi alanı içinde vegan uyarlamayı önceden konuşabileceğiniz en pratik duraklardan.", "vegan-options")), current(h("De Hoop op d'Swarte Walvis", "Hollanda ve deniz ürünü", "Zaanse Schans", "Balık, sebze ve Hollanda tabakları sunan tarihî görünümlü restoran.", "Sade balık/vegan siparişi için aday; helal sertifika, alkol ve ortak mutfak nedeniyle kesin değildir.", "verification-recommended", "netherlandsHalal")), "Zaanse Schans'ta sertifikalı helal restoran kanıtı sınırlıdır; Zaandam veya Amsterdam'da ana öğün planlamak daha güvenlidir."),
  guide("NL", "Keukenhof ve Lisse", current(v("Keukenhof Park Restaurants", "Park kafe ve vegan seçenekler", "Keukenhof", "Sezonluk park restoranlarında işaretli bitki bazlı sandviç ve sıcak seçenekler bulunabilir.", "Bahçe gününde resmî tesis içinde alerjen tablosu sorulabilen pratik yedek olduğu için.", "vegan-options")), current(h("Restaurant Den Ouden Heere", "Hollanda ve dünya", "Lisse", "Balık, sebze ve et yemekleri sunan merkezî restoran.", "Sade balık/vegan siparişi konuşulabilen aday; bağımsız helal sertifikası bulunmadığından ön teyit şart.", "verification-recommended", "netherlandsHalal")), "Keukenhof/Lisse'de sertifikalı helal seçenek kanıtı sınırlıdır; izin verilen ölçüde paketli öğün taşımak ve Leiden'da yemek planlamak daha güvenlidir."),
  guide("NL", "Texel", current(v("Paal 17", "Plaj restoranı ve vegan seçenekler", "De Koog güneyi", "Vegan burger, mercimek curry, kuskus ve dönemsel bitki bazlı tabakları bulunan plaj restoranı.", "Haziran 2026 yorumlarıyla birden fazla vegan ana yemeği güncel olarak doğrulanan ada seçeneği.", "vegan-options")), current(h("Bij Jef", "Ada ürünleri ve dünya", "Den Hoorn", "Yerel ürünlerle tadım menüsü hazırlayan restoran.", "Önceden vegan veya sade balık menüsü talep edilebilen aday; helal et ve alkolsüz hazırlık garantisi yazılı alınmalı.", "verification-recommended", "netherlandsHalal")), "Texel'de doğrulanmış sertifikalı helal restoran kanıtı sınırlıdır; Den Helder'den paketli sertifikalı gıda getirmek ve öz hazırlık yapabilen konaklama seçmek en güvenli plandır."),
